    ...
    if isinstance(periods, list):
        ...
        for period_index, period in enumerate(periods):
            if not isinstance(period, dict):
                continue
            # The browser parser preserves the canonical Periods ordering but
            # does not always include an explicit Number field.  Falling back
            # to zero collapses every set/half/period into p0 and lets a later
            # period overwrite the full-match market in the compatibility
            # projection.  The compatibility shim already uses the list index
            # for this legacy shape, so extraction must use the same mapping.
            pnum = period.get("Number", period_index)
