...
function oddsEquivalent(actual, expected, tolerance = 0.001) {
  const actualNumber = Number(actual);
  const expectedNumber = Number(expected);
  if (!Number.isFinite(actualNumber) || !Number.isFinite(expectedNumber)) return false;
  if (Math.abs(actualNumber - expectedNumber) <= tolerance) return true;
  // The UI renders decimal odds to two places while Paddy/implyBets retain
  // the exact fractional conversion (for example 4/3 => 1.333333 vs 1.33).
  return Math.abs(actualNumber - Number(expectedNumber.toFixed(2))) <= tolerance;
}
