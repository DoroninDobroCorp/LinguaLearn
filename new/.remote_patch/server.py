...
ROBINARB_ODDS_TOLERANCE = max(0.0001, float(os.getenv("ROBINARB_ODDS_TOLERANCE", "0.001")))
...
def _forted_delivery_alive(now: float | None = None) -> bool:
    ...
    if _forted_stream_alive(checked_at):
        return True
    return (
        _arbs_source in {"forted", "listener"}
...
async def _place_betfair_via_api(
    ...
    *,
    ...
    selection = str(quote.get("selection") or arb.get("bk2_selection") or arb.get("side2") or "").strip()
    try:
...
def _consume_verified_quote(username: str, arb_id: str, quote_id: str | None) -> dict[str, Any] | None:
    ...
    with _verified_quotes_lock:
        ...
        if float(quote.get("expires_at") or 0) <= now:
            ...
            return None
        return dict(_verified_quotes.pop(quote_id))
...
async def _robin_work_price_for_arb(arb: dict[str, Any]) -> tuple[float, str]:
    ...
    price_signature = ""
    if stream_lookup:
        stream_odds = _to_float_or_none(stream_lookup.get("decimal_odds"))
        if stream_odds is not None and stream_odds > 1:
            pin_odds = stream_odds
...
async def _prepare_betfair_sportsbook_requests(
    ...
    *,
    ...
    return await betfair_sportsbook_place_api.BetfairSportsbookPlaceApiClient().prepare(
        ...
        selection_id=quote.get("selection_id"),
        stake=stake,
        expected_odds=float(quote.get("current_odds")),
        event_url=event_url,
        market_name=str(quote.get("market_name") or arb.get("market") or ""),
        selection=str(quote.get("selection") or arb.get("bk2_selection") or arb.get("side2") or ""),
    )
...
async def place_bet(req: BetRequest, current_username: str = Depends(_require_current_username)):
    ...
    if not arb:
        ...
        arb = dict(snapshot) if isinstance(snapshot, dict) else None
    if not arb:
        raise HTTPException(404, "Arb not found")
    if not _verified_quote_matches_current_arb(quote, arb):
    ...
    live_place_required = (
        ...
        or (req.side == "robinbet" and verify_mode == "betslip" and _betfair_live_place_available())
...
async def api_reconcile_live_place(
    ...
    """Read-only: resolve a pending BIA/betslip order without re-placing."""
    result = await reconcile_pinnacle_live_place(order_id)
    status = str(result.get("status") or "").upper() if isinstance(result, dict) else ""
    with _users_lock:
        user = _users.get(current_username)
        if user:
            for bet in user.get("bets") or []:
                live = bet.get("pinnacle_live_place") or {}
                if str(live.get("order_id") or live.get("wager_id") or "") != str(order_id):
                    continue
                if status == "PLACED":
                    bet["status"] = "accepted"
                    live["status"] = "PLACED"
                    ...
                    live["status"] = status or live.get("status")
                    live["reconciliation_required"] = True
                bet["pinnacle_live_place"] = live
