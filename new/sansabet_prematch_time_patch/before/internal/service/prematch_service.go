...
// isWithin24Hours checks if match date is within 24 hours
// DP format examples: "03.12.2025 15:30" or "/Date(1733234400000)/"
func isWithin24Hours(dp string, cutoff time.Time) bool {
	if dp == "" {
		return true // Include if no date
	}

	// Try parsing .NET JSON date format: /Date(1733234400000)/ or /Date(1733234400000+0100)/
	if strings.Contains(dp, "/Date(") {
		msStr := dp[6 : len(dp)-2]
		// Remove timezone offset if present (e.g., +0100)
		if idx := strings.Index(msStr, "+"); idx > 0 {
			msStr = msStr[:idx]
		}
		if idx := strings.Index(msStr, "-"); idx > 0 && idx > 5 {
			msStr = msStr[:idx]
		}
		if ms, err := strconv.ParseInt(msStr, 10, 64); err == nil {
			matchTime := time.Unix(ms/1000, (ms%1000)*1000000)
			return matchTime.Before(cutoff)
		}
	}

	// Try parsing European date format with time: "03.12.2025 15:30"
	if matchTime, err := time.Parse("02.01.2006 15:04", dp); err == nil {
		return matchTime.Before(cutoff)
	}

	// Try parsing European date format without time: "03.12.2025" (assume end of day)
	if matchTime, err := time.Parse("02.01.2006", dp); err == nil {
		// Add 23:59 to include full day
		matchTime = matchTime.Add(23*time.Hour + 59*time.Minute)
		return matchTime.Before(cutoff)
	}

	// Try ISO format
	if matchTime, err := time.Parse(time.RFC3339, dp); err == nil {
		return matchTime.Before(cutoff)
	}
