...
import aggregator.fleet.worker as worker_module
...
from aggregator.fleet.worker import (
    MultiSportWorker,
    NormalizedEventDeltaCache,
    Worker,
    _browser_frame_timestamp,
    _fatal_auth_marker,
    _resnapshot_interval,
    _stamp_confirmed_market_ts,
    normalize_full_odds,
    normalize_browser_ws_frame,
...
def test_fatal_auth_marker_detects_suspended_account_before_positive_markers() -> None:
    body = (
        "15.23 EUR DEPOSIT user-name Your account has been suspended "
        "You are not permitted to see this page."
    )

    assert _fatal_auth_marker(body) == "account suspended"


def test_fatal_auth_marker_allows_normal_authenticated_page() -> None:
    assert _fatal_auth_marker("15.23 EUR DEPOSIT SPORTS Soccer") == ""
...
def _cached_live_event() -> dict[str, Any]:
    return {
        "Pid": 1630000001,
        "SportId": 29,
        "isLive": True,
        "PriceConfirmedAt": "old-frame",
        "Periods": [
            {
                "Number": 0,
                "Win1x2": {
                ...
                "_market_ts": {"Win1x2": 100.0, "Totals": 100.0},
            }
        ],
    }
...
def _compact_u_row(
    *,
    bet_type: int = 3,
    team_select: int = 3,
    handicap: float | None = 2.5,
    price: float = 2.05,
    line_id: int = 3623761001,
    is_alt: int = 0,
    event_id: int = 1630000001,
) -> list[Any]:
    return [0, bet_type, team_select, handicap, 0, price, line_id, 0, 0, 0, "O", is_alt, event_id]


def test_compact_u_updates_only_provenanced_leaf_and_exact_market_timestamp(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cache = NormalizedEventDeltaCache()
    cache.seed(29, [_cached_live_event()])
    monkeypatch.setattr(worker_module, "_price_confirmed_at", lambda ts: f"source:{ts}")

    changed = cache.apply(
        {"type": "UPDATE_ODDS", "time": 1780000010000, "odds": {"u": [[29, [_compact_u_row()]]]}},
        sport=29,
    )

    assert len(changed) == 1
    period = changed[0]["Periods"][0]
    assert period["Totals"]["2.5"]["WinMore"]["value"] == 2.05
    assert period["Totals"]["2.5"]["WinLess"]["value"] == 1.89
    assert period["_market_ts"] == {"Win1x2": 100.0, "Totals": 1780000010.0}
    assert changed[0]["PriceConfirmedAt"] == "source:1780000010.0"


def test_compact_u_unmatched_or_new_market_fails_closed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cache = NormalizedEventDeltaCache()
    cache.seed(29, [_cached_live_event()])
    ...
    frame = {
        ...
        "odds": {
            "u": [[29, [
                _compact_u_row(line_id=999999999),
                _compact_u_row(handicap=3.5, line_id=3623761001),
                _compact_u_row(price=0.0),
            ]]],
        },
    }

    assert cache.apply(frame, sport=29) == []


def test_compact_u_can_match_a_provenanced_child_line_without_rekeying_parent_pid(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    event = _cached_live_event()
    event["Periods"][0]["Totals"]["2.5"]["WinMore"]["raw"]["event_id"] = 1630000099
    event["Periods"][0]["Totals"]["2.5"]["WinLess"]["raw"]["event_id"] = 1630000099
    cache = NormalizedEventDeltaCache()
    cache.seed(29, [event])
    monkeypatch.setattr(worker_module, "_price_confirmed_at", lambda ts: f"source:{ts}")

    changed = cache.apply(
        {"type": "UPDATE_ODDS", "time": 1780000010000, "odds": {"u": [[29, [
            _compact_u_row(event_id=1630000099),
        ]]]}},
        sport=29,
    )

    assert changed[0]["Pid"] == 1630000001
    assert changed[0]["Periods"][0]["Totals"]["2.5"]["WinMore"]["value"] == 2.05


def test_compact_u_moneyline_null_handicap_matches_complete_snapshot_provenance(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cache = NormalizedEventDeltaCache()
    cache.seed(29, [_cached_live_event()])
    ...
    assert changed[0]["Periods"][0]["Win1x2"]["Win1"]["value"] == 2.20
    assert changed[0]["Periods"][0]["_market_ts"] == {
        "Win1x2": 1780000010.0,
        "Totals": 100.0,
    }


def test_normalize_browser_ws_frame_uses_worker_local_cache_for_compact_u(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    cache = NormalizedEventDeltaCache()
    cache.seed(29, [_cached_live_event()])
    ...
    changed = normalize_browser_ws_frame(
        {"type": "UPDATE_ODDS", "time": 1780000010000, "odds": {"u": [[29, [_compact_u_row()]]]}},
        29,
        cache,
    )
...
@pytest.mark.asyncio
async def test_worker_loop_emits_compact_u_from_its_local_snapshot_cache(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import json as _json

    original = _cached_live_event()
    monkeypatch.setattr(
        worker_module,
        "normalize_full_odds",
        lambda frame, sport: [original] if frame.get("type") == "FULL_ODDS" else [],
    )
    monkeypatch.setattr(worker_module, "_price_confirmed_at", lambda ts: f"source:{ts}")
    emitted: list[dict[str, Any]] = []
    worker = Worker(label="delta-cache", sport=29, slug="soccer", on_event=emitted.append)
    frames = [
        _json.dumps({"type": "FULL_ODDS", "odds": {"l": []}}),
        _json.dumps({"type": "UPDATE_ODDS", "time": 1780000010000, "odds": {"u": [[29, [_compact_u_row()]]]}}),
    ]
    ...
    await worker._loop(FakePg(), run_sec=0.02, watchlist=[])

    assert len(emitted) == 2
    assert emitted[1]["_account"] == "delta-cache"
    assert emitted[1]["Periods"][0]["Totals"]["2.5"]["WinMore"]["value"] == 2.05
    assert emitted[1]["Periods"][0]["_market_ts"]["Totals"] == 1780000010.0
...
def test_resnapshot_interval_has_opt_in_soccer_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PS3838_RESUBSCRIBE_PREMATCH_SEC", raising=False)
    monkeypatch.delenv("PS3838_RESUBSCRIBE_SOCCER_SEC", raising=False)

    assert _resnapshot_interval({}, sport=29) == 60.0
    assert _resnapshot_interval({}, sport=33) == 60.0

    monkeypatch.setenv("PS3838_RESUBSCRIBE_PREMATCH_SEC", "60")
    monkeypatch.setenv("PS3838_RESUBSCRIBE_SOCCER_SEC", "5")

    assert _resnapshot_interval({}, sport=29) == 5.0
    assert _resnapshot_interval({}, sport=33) == 60.0
    assert _resnapshot_interval({}) == 60.0
    assert _resnapshot_interval({"resnapshot_sec": "12"}, sport=29) == 12.0
...
@pytest.mark.asyncio
async def test_worker_uses_soccer_specific_resnapshot_interval(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PS3838_RESUBSCRIBE_PREMATCH_SEC", "60")
    monkeypatch.setenv("PS3838_RESUBSCRIBE_SOCCER_SEC", "0.2")
    page = _FakeFleetPage()
    worker = Worker(
        label="a0",
        sport=29,
...
@pytest.mark.asyncio
async def test_multi_sport_worker_applies_resnapshot_interval_per_page(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("PS3838_RESUBSCRIBE_PREMATCH_SEC", "60")
    monkeypatch.setenv("PS3838_RESUBSCRIBE_SOCCER_SEC", "0.2")
    soccer_page = _FakeFleetPage()
    tennis_page = _FakeFleetPage()
    worker = MultiSportWorker(
        label="a0",
        sports=[SportSpec("soccer", 29), SportSpec("tennis", 33)],
        on_event=lambda _event: None,
    ...
    await worker._loop_multi(
        [(soccer_page, 29, "soccer"), (tennis_page, 33, "tennis")],
        run_sec=0.65,
        watchlist=[],
    )

    assert len(soccer_page.sent) >= 1
    assert tennis_page.sent == []
...
def test_normalize_full_odds_parses_period_prices() -> None:
    ...
    assert event["Periods"][0]["_market_ts"] == {"Win1x2": 1780000000.0}


def test_stamp_confirmed_market_ts_only_marks_observed_positive_groups() -> None:
    event = {
        "Periods": [
            {
                "Win1x2": {"Win1": {"value": 1.8}},
                "Totals": {},
                "Handicap": {"-0.5": {"Win1": {"value": 2.0}}},
                "FirstTeamTotals": {"0.5": {"WinMore": {"value": 0}}},
                "SecondTeamTotals": {"0.5": {"WinLess": {"value": 0.5}}},
            }
        ]
    }

    _stamp_confirmed_market_ts(event, 100.0)

    assert event["Periods"][0]["_market_ts"] == {
        "Win1x2": 100.0,
        "Handicap": 100.0,
    }


def test_stamp_confirmed_market_ts_never_regresses_existing_timestamp() -> None:
    event = {
        "Periods": [
            {
                "Win1x2": {"Win1": {"value": 1.8}},
                "Totals": {"2.5": {"WinMore": {"value": 1.9}}},
                "_market_ts": {"Win1x2": 110.0},
            }
        ]
    }
...
@pytest.mark.parametrize("raw", [None, "1780000000000", float("nan"), -1, True])
def test_browser_frame_timestamp_rejects_invalid_values(raw: Any) -> None:
    assert _browser_frame_timestamp({"time": raw}, now=1780000001.0) is None


def test_browser_frame_timestamp_rejects_future_value() -> None:
    assert _browser_frame_timestamp(
        {"time": 1780000010000}, now=1780000000.0
    ) is None
