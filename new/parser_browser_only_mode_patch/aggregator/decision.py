...
Browser-only quote authority
----------------------------

``MSP_BROWSER_ONLY_AUTHORITATIVE=1`` is an explicit deployment policy for
installations where the authenticated browser WebSocket is the intended
top-level line source.  It does not start, enable, or probe any API.  The
system monitor may still report ``API_DEGRADED`` as topology telemetry, but a
fresh native ``BROWSER_WS`` winner is not stamped as a stale/degraded quote.
The flag is off by default so API-required deployments keep their existing
semantics.
...
def browser_only_authoritative_enabled() -> bool:
    """Whether fresh native browser-WS quotes are authoritative without API.

    This is a quote-quality policy only.  It deliberately does not change
    source startup, source health, or :class:`SystemModeMonitor` telemetry.
    """
    return os.environ.get("MSP_BROWSER_ONLY_AUTHORITATIVE", "").strip() in (
        "1",
        "true",
        "True",
        "yes",
...
class DecisionEngineV2:
    ...
    def __init__(
        ...
        *,
        ...
        browser_only_authoritative: bool = False,
    ) -> None:
        ...
        # Explicit deployment policy.  Default False is intentionally
        # backward-compatible for installations where the official API is a
        # required authority tier.
        self.browser_only_authoritative = bool(browser_only_authoritative)
    ...
    def _reason(
        ...
        *,
        ...
        if winner.authority >= min_native:
            ...
            browser_ws_is_expected_authority = (
                self.browser_only_authoritative
                and mode is SystemMode.API_DEGRADED
                and winner.authority is AuthorityClass.BROWSER_WS
            )
            degraded = (
                mode is not SystemMode.NORMAL
                and not browser_ws_is_expected_authority
            )
            fallback_state = None if not degraded else _FALLBACK_STATE_BY_MODE[mode]
...
def build_default_engine() -> DecisionEngine | DecisionEngineV2:
    ...
    if decision_v2_enabled():
        return DecisionEngineV2(
            browser_only_authoritative=browser_only_authoritative_enabled()
        )
...
__all__ = [
    ...
    "DecisionEngine",
    "DecisionEngineV2",
    "browser_only_authoritative_enabled",
