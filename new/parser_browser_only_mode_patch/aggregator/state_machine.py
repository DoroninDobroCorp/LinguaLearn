...
class SystemModeMonitor:
    ...
    ``API_DEGRADED`` remains the topology/operations signal when an API is
    absent.  A browser-only deployment may separately treat a fresh
    browser-WS winner as an authoritative, non-stale *quote* via the decision
    engine's explicit browser-only policy; that policy intentionally does not
    rewrite this monitor's telemetry.
    """
