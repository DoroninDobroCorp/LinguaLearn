...
_CACHE_TTL_SEC = 30.0
_PENDING_MORE_BET_TTL_SEC = 90.0
_BIA_PROOF_IDENTITY_TTL_SEC = 15 * 60.0
_HEALTH_MAX_SOURCE_AGE_SEC = max(
    5.0,
    float(os.getenv("MSP_HUB_HEALTH_MAX_SOURCE_AGE_SEC", "90")),
)


def _offer_event_debug_payload(
    offer_event: dict[str, Any],
    identity: tuple[str, str],
    *,
    observer_collision: bool,
    observed_aux_collision: bool,
) -> dict[str, Any]:
    """Expose bounded structural proof evidence without bookmaker prices."""
    return {
        "sport_code": identity[0],
        "event_key": identity[1],
        "competition_id": offer_event.get("competition_id"),
        "competition_count": int(offer_event.get("competition_count") or 0),
        "collision": bool(offer_event.get("collision")),
        "observer_collision": bool(observer_collision),
        "observed_aux_collision": bool(observed_aux_collision),
...
_SPECIAL_TYPE_RE = re.compile(r"^[a-z][a-z0-9_]{0,63}$")
_SPECIAL_STATE_MARKETS: dict[str, str] = {
    "btts": "BTTS",
    ...
    "home_team_to_score": "HomeTeamToScore",
    "away_team_to_score": "AwayTeamToScore",
    "either_team_to_score": "EitherTeamToScore",
    "home_win_to_nil": "HomeWinToNil",
    "away_win_to_nil": "AwayWinToNil",
    "to_qualify": "ToQualify",
    "winning_margin": "WinningMargin",
    "total_goals_range": "TotalGoalsRange",
    "exact_total_goals": "ExactTotalGoals",
    "home_exact_goals": "HomeExactGoals",
    "away_exact_goals": "AwayExactGoals",
    "winner_total_combo": "WinnerTotalCombo",
    "btts_winner_combo": "BTTSWinnerCombo",
    "btts_total_combo": "BTTSTotalCombo",
    "odd_even_total_combo": "OddEvenTotalCombo",
}


def _period_payload(event: dict[str, Any], period: int) -> dict[str, Any] | None:
    periods = event.get("Periods")
    if isinstance(periods, list):
        for item in periods:
            if isinstance(item, dict) and _to_int(item.get("Number")) == period:
                return item
    legacy = event.get("Period")
    if isinstance(legacy, dict):
        item = legacy.get(period, legacy.get(str(period)))
        if isinstance(item, dict):
            return item
    return None


def _normalized_special_offer(
    events_data: dict[Any, Any],
    event_id: int,
    special_type: str,
    contestant: str,
    period: int,
    ...
    *,
    now: float | None = None,
    ttl_sec: float = 30 * 60.0,
) -> dict[str, Any] | None:
    """Read one exact flat special from the normalized Pinnacle/BIA event."""
    if handicap != 0:
        return None
    market_key = _SPECIAL_STATE_MARKETS.get(special_type)
    if not market_key:
        return None
    event = events_data.get(event_id, events_data.get(str(event_id)))
    if not isinstance(event, dict):
        return None
    period_data = _period_payload(event, period)
    if not isinstance(period_data, dict):
        return None
    market = period_data.get(market_key)
    if not isinstance(market, dict) or contestant not in market:
        return None
    observed_at = _to_float(period_data.get(f"_{market_key}_ts"))
    current = time.time() if now is None else float(now)
    if observed_at is None or current < observed_at or current - observed_at > ttl_sec:
        return None
    odd = market.get(contestant)
    price = _positive_price(odd.get("value") if isinstance(odd, dict) else odd)
    if price is None:
        return None
    structural_id = ":".join((
        "bia-special-proof",
...
def special_lookup_payload(
    event_id_raw: Any,
    special_type_raw: Any,
    contestant_raw: Any,
    period_raw: Any = 0,
    handicap_raw: Any = 0,
    *,
    lookup: Callable[..., dict[str, Any] | None] | None = None,
    events_data: dict[Any, Any] | None = None,
    proof_only: bool = False,
) -> tuple[dict[str, Any], int]:
    """Return one fresh, structurally exact Pinnacle special selection.

    The lookup key is event + special type + contestant + period + handicap.
    Odds are returned only after that identity is resolved; they never
    participate in choosing a market or contestant.
    """
    try:
        event_id = int(event_id_raw)
        period = int(period_raw or 0)
        ...
        return {
            "error": "event_id, period and handicap must be numeric",
        }, 400
    special_type = str(special_type_raw or "").strip().lower()
    contestant = str(contestant_raw or "").strip()
    if event_id <= 0:
        return {"error": "event_id is required"}, 400
    if period < 0 or not math.isfinite(handicap):
        return {"error": "invalid special coordinates"}, 400
    if not _SPECIAL_TYPE_RE.fullmatch(special_type):
        return {"error": "invalid special type"}, 400
    if not contestant or len(contestant) > 160:
        return {"error": "contestant is required"}, 400
    if proof_only:
        # BIA-only callers need an address, not a cached parser/observer
        # price. The handler continues from this price-free miss into the raw
        # BIA offer-proof registry and finally obtains the current price from
        # a real BIA betslip. Legacy callers retain the cached lookup below.
        return {
            "found": False,
            "event_id": event_id,
    ...
    use_runtime_state = lookup is None and events_data is None
    if lookup is None:
        from special_ids_store import lookup_contestant_id
        lookup = lookup_contestant_id
    result = lookup(event_id, special_type, contestant, period, handicap)
    if not isinstance(result, dict):
        if use_runtime_state:
            from state import state
            events_data = state.events_data
        if events_data is not None:
            result = _normalized_special_offer(
                events_data,
                event_id,
                special_type,
                contestant,
    ...
    price = _positive_price(result.get("price"))
    ...
    return {
        ...
        "handicap": handicap,
        "cid": cid,
        "special_id": result.get("special_id"),
        "price": price,
        "ts": result.get("ts"),
        "source": str(result.get("source") or "pinnacle_special_ids"),
    }, 200


def special_offer_proof_payload(
    ...
    *,
    ...
    period: int,
    handicap: float,
    collision_identities: set[tuple[str, str]] | None = None,
) -> dict[str, Any]:
    """Resolve one special against exactly one raw BIA event namespace."""
    normalized_collision_identities = {
        (str(sport or ""), str(event or ""))
        for sport, event in (collision_identities or set())
    }
    if any(
        (
            str(event_ref.get("sport_code") or ""),
            str(event_ref.get("event_key") or ""),
        ) in normalized_collision_identities
        for event_ref in event_refs
    ):
        return {
            "found": False,
            "event_found": bool(event_refs),
    ...
    for event_ref in event_refs:
        proof = registry.try_prove_special(
            event_ref,
            special_type=special_type,
            contestant=contestant,
            period=period,
        ...
        if proof.get("status") == "OK":
            successes.append((event_ref, proof))
        elif isinstance(proof.get("error_code"), str):
            failure_codes.add(str(proof["error_code"]))
    if "BIA_OFFER_EVENT_COLLISION" in failure_codes:
        return {
            "found": False,
            "event_found": bool(event_refs),
            "error_code": "BIA_OFFER_EVENT_COLLISION",
            "candidate_count": len(event_refs),
            "candidate_error_codes": sorted(failure_codes),
        }

    def physical_event_key(event_ref: dict[str, Any]) -> str:
        raw = str(event_ref.get("event_key") or "").strip()
        parts = [part.strip() for part in raw.split(",")]
        if len(parts) == 3 and parts[0] and parts[1].isdigit() and parts[2].isdigit():
            return ",".join((parts[0], *sorted(parts[1:])))
        return raw

    physical_event_keys = {physical_event_key(event_ref) for event_ref in event_refs}
    if successes and len(successes) < len(event_refs) and len(physical_event_keys) > 1:
        return {
            "found": False,
            "event_found": True,
            "error_code": "BIA_EVENT_SELECTION_INCOMPLETE",
            "candidate_count": len(event_refs),
            "proven_candidate_count": len(successes),
            "incomplete_candidate_count": len(event_refs) - len(successes),
            "candidate_error_codes": sorted(failure_codes),
        }
    if len(successes) > 1:
        return {
            "found": False,
            "event_found": True,
            "error_code": "BIA_SPECIAL_OFFER_AMBIGUOUS",
    ...
    if not successes:
        return {
            ...
            "error_code": (
                ...
                else "BIA_SPECIAL_OFFER_NOT_FOUND"
            ),
            "candidate_count": len(event_refs),
        }
    event_ref, proof = successes[0]
    return {
        "found": True,
...
class Pin888HubCompatState:
    ...
    def __init__(
        ...
        *,
        ...
        pending_more_bet_ttl_sec: float = _PENDING_MORE_BET_TTL_SEC,
        health_max_source_age_sec: float = _HEALTH_MAX_SOURCE_AGE_SEC,
    ) -> None:
        self.cache_ttl_sec = float(cache_ttl_sec)
        self.pending_more_bet_ttl_sec = max(1.0, float(pending_more_bet_ttl_sec))
        self.health_max_source_age_sec = max(1.0, float(health_max_source_age_sec))
    ...
    def ingest_event(self, event: dict[str, Any]) -> list[list[Any]]:
        ...
        # Some upstream normalized payloads retain two versions of the same
        # period after a root-line move.  The later period is the current one;
        # expose one price per exact structural coordinate to consumers.
        rows = list({
            (row[-1], row[0], row[1], row[2], row[3]): row
            for row in rows
        }.values())
        if not rows:
            return []
        ...
        with self._condition:
            ...
            event_id = _to_int(event.get("Pid"))
            if event_id:
                # One normalized event is a complete board snapshot.  Line ids
                # change whenever Pinnacle moves a root line, so key-by-line-id
                # upserts alone retain the previous board beside the new one.
                # Consumers can then resolve an exact outcome against an old
                # copy and advertise a price that is no longer current.
                for rows_by_key in (bucket, target, opposite):
                    stale_keys = [
                        key
                        for key, existing_row in rows_by_key.items()
                        if _to_int(existing_row[-1]) == event_id
                    ]
    ...
    def health(self, *, consumers: int = 0) -> dict[str, Any]:
        ...
        source_ages = [
            ...
            if item.get("rows") and item.get("age_sec") is not None
        ]
        source_age = min(source_ages) if source_ages else None
        any_source_fresh = bool(
            source_age is not None
            and source_age <= self.health_max_source_age_sec
        )
        stale_sports = sorted(
            slug
            for slug, item in sports.items()
            if item.get("rows")
            and item.get("age_sec") is not None
            and float(item["age_sec"]) > self.health_max_source_age_sec
        )
        source_fresh = any_source_fresh and not stale_sports
        health_reason = (
            "fresh"
            if source_fresh
            else "source_inventory_empty"
            if source_age is None
            else "stale_sports"
        )
        return {
            # Having cached rows is not health.  A disconnected parser used
            # to keep returning ``ok=true`` for hours, which made consumers
            # trust an attractive but frozen Pinnacle board.  At least one
            # populated sport must have received a current source update.
            "ok": source_fresh,
            "source": "central-ps3838-hub-compat",
            "source_fresh": source_fresh,
            "any_source_fresh": any_source_fresh,
            "health_reason": health_reason,
            "source_age_sec": round(source_age, 3) if source_age is not None else None,
            "health_max_source_age_sec": self.health_max_source_age_sec,
            "stale_sports": stale_sports,
            "sports": sports,
            "consumers": consumers,
        ...
        *,
        ...
        with self._condition:
            ...
            if cached is not None:
                ...
                body["cached"] = True
                return body
            self._queue_morebet_locked(key, priority=priority)
            while True:
    ...
    def queue_morebet_target(self, event_id: Any, *, priority: bool = False) -> bool:
        key = str(event_id or "").strip()
        if not key:
            return False
        with self._condition:
    ...
    def next_morebet_target(self) -> int | None:
        ...
        with self._condition:
            for key, pending in sorted(
                ...
                key=lambda item: (
                    -item[1].priority,
                    item[1].last_dispatched_at > 0.0,
                    item[1].last_dispatched_at,
                    item[1].requested_at,
                ),
            ):
                if self._fresh_more_bet_locked(key) is not None:
                    self._pending_more_bets.pop(key, None)
                    continue
                if now - pending.requested_at > self.pending_more_bet_ttl_sec:
                    self._pending_more_bets.pop(key, None)
                    continue
    ...
    def _queue_morebet_locked(self, key: str, *, priority: bool = False) -> None:
        pending = self._pending_more_bets.setdefault(
            key,
            _PendingMoreBet(requested_at=time.time()),
        )
        if priority:
            pending.priority = 1
        self._condition.notify_all()
...
class Pin888HubCompatServer:
    ...
    def _run(self) -> None:
        ...
        app.router.add_get("/lookup-special", self._handle_lookup_special)
    ...
    async def _handle_more_bet(self, request: web.Request) -> web.Response:
        ...
        priority = str(request.query.get("priority") or "").strip().lower() in {
            "1", "true", "high", "selected",
        }
        body = await asyncio.to_thread(
            self.state.request_more_bet,
            event_id,
            timeout=timeout,
            priority=priority,
        )
    ...
    async def _handle_debug_event(self, request: web.Request) -> web.Response:
        ...
        if stats is not None:
            observer_collision_identities = (
                bia_observer._observer_registry_collision_identities(stats)
            )
            observed_aux_collision_identities = set(
            ...
            for offer_event in stats._offer_proofs.snapshot().get("events", []):
                ...
                if identity in ref_identities:
                    offer_groups.append(_offer_event_debug_payload(
                        offer_event,
                        identity,
                        observer_collision=(
                            identity in observer_collision_identities
                        ),
                        observed_aux_collision=(
                            identity in observed_aux_collision_identities
                        ),
                    ))
        return web.json_response({
            "found": True,
    ...
    @staticmethod
    def _seed_bia_identity_from_request(request: web.Request) -> bool:
        """Seed a price-free parser identity for either exact lookup route."""
        ...
        if not (pinnacle_home and pinnacle_away and pinnacle_sport):
            return False
        from state import state

        return seed_bia_proof_identity(
            state.events_data,
    ...
    async def _handle_lookup_special(self, request: web.Request) -> web.Response:
        # Forted/Pinnacle participant metadata may propose a child event while
        # the central parser inventory is unavailable. The independent BIA
        # registry and raw special group still have to prove the exact event
        # and selection; a real BIA betslip proves the current price later.
        # This seed alone never verifies anything.
        self._seed_bia_identity_from_request(request)
        ...
        body, status = special_lookup_payload(
            ...
            request.query.get("contestant"),
            request.query.get("period", "0"),
            request.query.get("handicap", "0"),
            proof_only=proof_requested,
        )
        if status == 200 and not body.get("found"):
            try:
                event_id = int(request.query.get("event_id") or 0)
                period = int(request.query.get("period") or 0)
                handicap = float(request.query.get("handicap") or 0)
                from services import bia_observer
                from services.bia_observer import _matching_bia_event_refs_for_pid

                refs = _matching_bia_event_refs_for_pid(event_id)
                stats = bia_observer._current_stats
                if stats is not None:
                    body = special_offer_proof_payload(
                        refs,
                        stats._offer_proofs,
                        special_type=str(request.query.get("type") or "").strip().lower(),
                        contestant=str(request.query.get("contestant") or "").strip(),
                        period=period,
                        handicap=handicap,
            ...
            try:
                ...
                body = await lookup_bia_selection_for_pid_with_refresh(
                    ...
                    selection={
                        "special_type": str(request.query.get("type") or "").strip().lower(),
                        "contestant": str(request.query.get("contestant") or "").strip(),
                        "handicap": request.query.get("handicap", "0"),
                    },
                )
                if body.get("found"):
                    body = {"source": "bia_special_offer_proof", **body}
            except (TypeError, ValueError):
                pass
        ...
        if status == 200 and proof_requested and not body.get("found"):
            try:
                ...
                body = await lookup_bia_selection_for_pid_with_refresh(
                    int(request.query.get("event_id") or 0),
                    period=int(request.query.get("period") or 0),
                    allow_stale_candidate=str(
                        request.query.get("stale_candidate") or ""
