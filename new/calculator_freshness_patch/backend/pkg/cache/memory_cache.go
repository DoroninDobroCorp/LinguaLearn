...
type MemoryCacheInterface[K key, V any] interface {
	...
	Lock()
	Unlock()
	// Optimized iteration without copying entire map
	Iterate(fn func(K, V) bool)
	IterateUnsafe(fn func(K, V) bool)
}
...
// DeleteBatch removes multiple keys in one operation (more efficient than item-by-item)
// TEST: Verify no data loss - keys deleted should actually be expired/invalid
func (m *MemoryCache[K, V]) DeleteBatch(keys []K) {
	if len(keys) == 0 {
		return
	}
...
// Iterate iterates over cache with RLock held during entire iteration
// WARNING: Do NOT acquire any locks inside fn() - risk of deadlock!
// TEST: Ensure no deadlocks when using this method
func (m *MemoryCache[K, V]) Iterate(fn func(K, V) bool) {
	m.m.RLock()
	defer m.m.RUnlock()
...
// IterateUnsafe iterates without locking - caller must hold lock!
func (m *MemoryCache[K, V]) IterateUnsafe(fn func(K, V) bool) {
	for k, v := range m.data {
		if !fn(k, v) {
			break
		}
