...
from sender_analyzer import SenderToAnalyzer, safe_endpoint
...
class GGBetParser:
    def __init__(self):
        ...
        self.stats = {
            ...
            "last_catalog_count": None,
            "last_query_data_responses": 0,
            "last_query_error_responses": 0,
            "last_query_completions": 0,
            "last_query_expected": 0,
            "last_query_timed_out": False,
        }
    ...
    async def _fetch_all_events(self, ws) -> list:
        ...
        self.stats["last_catalog_count"] = len(all_events)
        self.stats["last_query_data_responses"] = data_responses
        self.stats["last_query_error_responses"] = error_responses
        self.stats["last_query_completions"] = completed
        self.stats["last_query_expected"] = target
        self.stats["last_query_timed_out"] = timed_out
        if all_events:
            self.stats["last_feed_state"] = "events"
        elif data_responses == target and completed == target and error_responses == 0 and not timed_out:
            self.stats["last_feed_state"] = "upstream_empty"
        elif data_responses > 0:
            self.stats["last_feed_state"] = "partial_or_empty"
        else:
            self.stats["last_feed_state"] = "query_failed"
...
def _health_state(stats: dict, last_update_age: Optional[float]) -> tuple[bool, str]:
    """Return consumer readiness and an operator-readable reason.

    A connected GraphQL socket is not enough: an analyzer consumer needs at
    least one usable event from a fresh cycle.  An official, successfully
    queried empty catalog therefore remains degraded, but is distinguished from
    a transport or GraphQL failure.
    """
    if not stats.get("connected"):
        return False, "disconnected"

    freshness_limit = max(30, FETCH_INTERVAL * 5, int(EMPTY_FETCH_INTERVAL * 1.5))
    if last_update_age is None or last_update_age > freshness_limit:
        return False, "cycle_stale"

    if stats.get("events_tracked", 0) <= 0:
        feed_state = stats.get("last_feed_state")
        if feed_state == "upstream_empty":
            return False, "upstream_empty"
        if feed_state == "query_failed":
            return False, "query_failed"
        return False, "no_usable_events"
...
async def health_handler(request):
    if parser_instance:
        ...
        ready, reason = _health_state(stats, last_update_age)
        payload = {
...
async def main():
    ...
    logger.info("Sender endpoint: %s", safe_endpoint(SENDER_URL))
