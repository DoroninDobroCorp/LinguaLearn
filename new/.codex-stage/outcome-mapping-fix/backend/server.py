...
ROBINARB_ROBIN_WORK_MORE_BET_TIMEOUT_SEC = max(
    ...
    min(
        ...
        float(os.getenv("ROBINARB_ROBIN_WORK_MORE_BET_TIMEOUT_SEC", "20.0")),
    ),
)
...
_SYSTEM_REJECTION_CATEGORIES = {"verification", "safety_filter", "feed_filter"}
...
def _ensure_esports_scope_metadata(arb: dict[str, Any]) -> dict[str, Any]:
    ...
    if not scope:
        return arb.get("pinnacle_market_metadata") if isinstance(arb.get("pinnacle_market_metadata"), dict) else {}
...
def _normalize_pinnacle_bia_transport_payload(
    ...
    *,
    ...
    for key in ("map_number", "esports_unit", "period_type"):
...
_FORTED_MARKET_PERIOD_RE = re.compile(
    r"(?:^|\s)(\d+)\s*(пер|и|[тпчс])(?:\s|$)",
    re.IGNORECASE,
)
_FORTED_ESPORTS_MAP_RE = re.compile(r"(?:^|\s)(\d+)\s*к(?:\s|$)", re.IGNORECASE)


def _forted_market_code_metadata(value: Any, sport: Any = "") -> dict[str, Any]:
    """Decode Forted's independent SB market coordinate.

    ``Nк`` is Forted's esports map marker (for example ``kills 2к``), not a
    decimal odd or a total value.  Baseball uses ``Nи`` for inning N, while
    Pinnacle MORE_BET addresses inning N as period N+2 (period 1 is first
    five innings and period 2 is Pinnacle's additional aggregate slot).
    Keep the map grammar sport-gated so unrelated bookmaker codes cannot
    accidentally acquire a map coordinate.
    """
    code = str(value or "").strip().lower().replace("ё", "е")
    sport_text = str(sport or "").strip().lower().replace("ё", "е")
    if any(token in sport_text for token in ("esport", "e-sport", "киберспорт")):
        map_match = _FORTED_ESPORTS_MAP_RE.search(code)
        if map_match:
            map_number = _to_int_or_none(map_match.group(1))
            if map_number is not None and 1 <= map_number <= 9:
                return {
                    "map_number": map_number,
                    "period_number": map_number,
                    "period_type": "map",
    ...
    if marker == "и":
        if not any(token in sport_text for token in ("baseball", "бейсбол")):
            return {}
        return {
...
def _pinnacle_result_matches_request(payload: dict[str, Any], result: dict[str, Any]) -> bool:
    ...
    for metadata_key, result_keys in comparable_fields:
        if (
            ...
            and str(metadata.get("period_type") or "").strip().lower() == "map"
        ):
...
_DRAW_ATOMS = frozenset({"1", "x", "2"})


def _draw_outcome_atoms(value: Any) -> frozenset[str] | None:
    clean = str(value or "").strip()
    if not clean:
        return None
    translated = _forted_translate_outcome(clean)
    token = str(translated or clean).strip().lower().replace("х", "x")
    token = re.sub(r"^p\s*\d+\s+", "", token)
...
def _has_exact_draw_partition(arb: dict[str, Any]) -> bool:
    pairs: list[tuple[Any, Any]] = []
    metadata = arb.get("pinnacle_market_metadata")
    if isinstance(metadata, dict):
        raw_stake_types = str(metadata.get("raw_stake_types") or "")
        raw_parts = [part.strip() for part in raw_stake_types.split(";") if part.strip()]
        if len(raw_parts) == 2:
            pairs.append((raw_parts[0], raw_parts[1]))
    pairs.append((
        arb.get("bk1_selection") or arb.get("side1"),
        arb.get("bk2_selection") or arb.get("side2"),
    ))
    for first, second in pairs:
        first_atoms = _draw_outcome_atoms(first)
        second_atoms = _draw_outcome_atoms(second)
        if (
            first_atoms
            and second_atoms
            and first_atoms.isdisjoint(second_atoms)
            and first_atoms | second_atoms == _DRAW_ATOMS
        ):
            return True
    return False
...
def _is_draw_prone_moneyline_arb(arb: dict[str, Any]) -> bool:
    ...
    return not _has_exact_draw_partition(arb)
...
def _issue_verified_quote(
    ...
    quote = {
        ...
        "verification_mode": verification_mode if verification_mode in {"stream", "betslip"} else None,
        "expires_at": now + ttl,
    }
...
async def lifespan(app: FastAPI):
    ...
    global _system_rejection_queue_stopping
...
def _get_current_system_rejections_page(
    *,
    ...
    category_order = {"verification": 0, "safety_filter": 1, "feed_filter": 2}
    candidates.sort(key=lambda item: (
        category_order.get(str(item.get("category") or ""), 3),
        -float(item.get("last_seen") or 0.0),
    ))
    return candidates[:bounded_limit], len(candidates)
...
async def list_verification_rejections(
    ...
    items, total_count = _get_current_system_rejections_page(
        now=now,
...
async def _more_bet_margin_price_for_robin_work(
    ...
    *,
    ...
    if not (
        ...
        return None
    robin_odds = robin_margin.compute_robin_odds(selected_odds, margin)
    ...
    return robin_odds, "pinnacle-more-bet"
...
async def _pinnacle_compact_margin_price_for_robin_work(
    ...
    *,
    ...
    if margin is None or margin < 0 or selected_odds is None or selected_odds <= 1 or not selected_line_id or not signature:
        _market_margin_cache_set(cache_lookup_key, None)
        return None
    robin_odds = robin_margin.compute_robin_odds(selected_odds, margin)
...
async def _pinnacle_exact_verify_price_for_robin_work(
    ...
    *,
    ...
    if _PINNACLE_EXACT_VERIFY_RETRY_AFTER.get(retry_key, 0.0) > now:
        return None
    ...
    try:
        response_obj = await _pinnacle_service_post("/verify", payload, scope="robin-work-verify", wait=False)
    ...
    for candidate in results:
        ...
        return robin_margin.fallback_by_odds(current_odds), "pinnacle-exact-verify"
    safe_upstream_code = _strict_external_diagnostic_code(upstream_code)
    _set_robin_work_verification_diagnostic(
        arb,
        status="blocked",
        ...
        retryable=safe_upstream_code in {"BIA_VERIFY_UNAVAILABLE", "EVENT_NOT_FOUND"},
...
async def _robin_work_price_for_arb(arb: dict[str, Any]) -> tuple[float, str]:
    ...
    # First choice: Arcadia's exact related matchup + exact market key.  Both
    # the selected price and the margin come from Pinnacle; Forted odds are not
    # involved in identity or calculation.
    ...
    if arcadia_odds is not None and arcadia_odds > 1 and arcadia_margin is not None and arcadia_margin >= 0 and arcadia_key:
        signature = f"arcadia:{arcadia.get('event_id')}:{arcadia_key}"
        robin_odds = robin_margin.compute_robin_odds(arcadia_odds, arcadia_margin)
        robin_margin.cache_robin_odds(
            cache_key,
            arcadia_odds,
            robin_odds,
    ...
    if stream_odds is not None and stream_odds > 1 and stream_margin is not None and stream_margin >= 0 and stream_signature:
        robin_odds = robin_margin.compute_robin_odds(stream_odds, stream_margin)
        robin_margin.cache_robin_odds(
            cache_key,
            stream_odds,
            robin_odds,
        ...
        _clear_robin_work_verification_diagnostic(arb, status="verified")
        return robin_odds, "pinnacle-stream-id"
...
async def _apply_robin_work_pricing(arbs: list[dict[str, Any]], enabled: bool) -> list[str]:
    ...
    _done, timed_out_tasks = await asyncio.wait(
        pricing_tasks,
        timeout=ROBINARB_ROBIN_WORK_PRICING_TIMEOUT_SEC,
    )
    if timed_out_tasks:
        ...
        log.warning(
            "RobinWork pricing timed out after %.1fs for %d/%d candidates; using safe fallback prices",
            ROBINARB_ROBIN_WORK_PRICING_TIMEOUT_SEC,
            len(timed_out_tasks),
            len(pricing_tasks),
        )
    ...
    for bookmaker_key in sorted(baselines_by_bookmaker):
        ...
        selected_rows = [
            ...
            if row[6] in _ROBIN_WORK_EXACT_PRICE_SOURCES
            and not row[4].get("robin_work_verification_blocked")
        ][:ROBINARB_ROBIN_WORK_TOP_N]
        selected_rank = {
            arb_id: rank
...
async def get_arbs(
    ...
    for arb in all_arbs:
        if not _visible_for_frontend_overvalue(arb):
            system_rejection_items.append(_safe_system_rejection_from_arb(
                arb,
                category="safety_filter",
...
async def verify_price(req: VerifyRequest, current_username: str = Depends(_require_current_username)):
    ...
    if PINNACLE_API_BASE and service_outcome and (event_id_int or has_pinnacle_identifier):
        ...
        if matched_candidate is not None and matched_odds is not None:
            ...
            # Resolve Robin independently, then require it to be based on the
            # exact same Pinnacle price.  A calculator response must never mix
            # (for example) a BIA PIN quote from one tennis related matchup
            # with an Arcadia Robin quote from another.
            working = dict(arb)
            ...
            if not robin_quote_verified and robin_odds is not None:
                log.warning(
                    "Robin quote withheld for arb=%s: PIN=%s RobinBase=%s source=%s key=%s",
                    req.arb_id, matched_odds, robin_base_odds, source, robin_market_key,
                )
                robin_odds = None
                source = "verify-quote-conflict"
            ...
            if robin_quote_verified and robin_odds is not None and robin_odds > 1:
                ...
                response_payload["robin_quote_detail"] = str(
                    working.get("robin_work_verification_block_reason")
                    or "Robin quote withheld because its structural Pinnacle binding did not match the verified betslip outcome"
                )
...
async def place_bet(req: BetRequest, current_username: str = Depends(_require_current_username)):
    ...
    if quote_verify_mode in {"stream", "betslip"}:
