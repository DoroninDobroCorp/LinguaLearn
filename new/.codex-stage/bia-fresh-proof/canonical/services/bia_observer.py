...
_EXACT_REFRESH_CONNECT_TIMEOUT_SEC = 0.75
...
def _fresh_exact_refresh_result(
    ...
    if len(request.candidate_keys) > 1:
        return {
            "found": False,
            "event_found": True,
            "event_id": request.event_id,
    ...
    watch_key = next(iter(request.candidate_keys))
...
def _record_fresh_exact_offer(
    ...
    *,
    ...
    if not isinstance(event_header, list) or len(event_header) < 3:
        return
    watch_key = _canonical_refresh_watch_key(event_header)
...
def _observer_registry_collision_identities(
    ...
    identity_competitions: dict[tuple[str, str], set[str]] = defaultdict(set)
    for comp_id, sport_code, event_key in list(stats._event_registry):
        identity = (
            str(sport_code).strip().lower(),
            str(event_key).strip(),
        )
        identity_competitions[identity].add(str(comp_id).strip())
    stats._collision_identities = {
        identity
        for identity, competition_ids in identity_competitions.items()
