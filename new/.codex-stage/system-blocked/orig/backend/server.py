...
async def _stream_lookup_for_robin_work(
    ...
    if _arb_market_context(arb):
        return None
    verify_payload = _build_pinnacle_verify_payload(arb)
    if not any(_payload_any_pin_ids(verify_payload).values()):
        return None
    try:
        lookup = await pinnacle_hub.lookup_stream_price(
...
async def _pinnacle_exact_verify_price_for_robin_work(
    ...
    *,
    ...
    try:
        response = await _pinnacle_service_post("/verify", payload, scope="robin-work-verify", wait=False)
        body = response.json()
        response.raise_for_status()
    except Exception as exc:
...
async def _robin_work_price_for_arb(arb: dict[str, Any]) -> tuple[float, str]:
    ...
    if pin_odds <= 1:
        ...
        arb["robin_work_verification_block_reason"] = "Forted did not provide a valid reference price"
    ...
    # binds the row.  With that binding we may use both current Pinnacle price
    # and its exact paired-market margin.
    ...
    if exact_verify is not None:
        ...
        arb["robin_work_verification_blocked"] = True
        arb["robin_work_verification_block_reason"] = (
            "Pinnacle selection was verified, but its exact paired-market margin is unavailable"
        )
        return exact_verify
    ...
    arb["robin_work_verification_blocked"] = True
    arb["robin_work_verification_block_reason"] = (
        "Pinnacle did not return an exact event / market key / line id for this outcome"
    )
...
def _robin_work_verification_block_reason(arb: dict[str, Any]) -> str:
    ...
    if str(arb.get("sport") or "").strip().lower() == "tennis" and family == "Game Winner":
        if not _to_int_or_none(metadata.get("set_number")):
            return "Forted did not provide the tennis set number required for an exact BIA game quote"
    ...
    if str(arb.get("sport") or "").strip().lower() == "esports":
        scope = _esports_scope_metadata(arb)
        if scope.get("esports_unit") and not _to_int_or_none(metadata.get("map_number")):
            return "Forted did not provide the esports map number required for an exact Pinnacle market quote"
...
async def _apply_robin_work_pricing(arbs: list[dict[str, Any]], enabled: bool) -> list[str]:
    ...
    for idx, (bookmaker_key, baseline, pin_profit, arb_id, arb) in enumerate(to_price_info):
        ...
        if isinstance(res, asyncio.TimeoutError):
            ...
            source = "unverified"
            arb["robin_work_verification_blocked"] = True
            arb["robin_work_verification_block_reason"] = "Exact Pinnacle verification timed out"
