...
def _enrich_result(
    ...
    *,
    ...
    map_number = params.get("map_number")
    if map_number not in (None, ""):
        # BIA map scope lives in tmap,N on the root event.  Do not mirror the
        # caller's Pinnacle child period back as the verified event period.
        period_num = 0
    ...
    if map_number not in (None, ""):
        enriched.setdefault("map_number", int(map_number))
        esports_unit = str(params.get("esports_unit") or "").strip().lower()
        if esports_unit:
            enriched.setdefault("esports_unit", esports_unit)
    tennis_unit = str(params.get("tennis_unit") or "").strip().lower()
    if tennis_unit:
        enriched.setdefault("tennis_unit", tennis_unit)
    if bt in (2, 3, 4, 5):
...
async def market_margin(request: Request, req: VerifyRequest) -> dict:
    ...
    assert cache is not None
    # A known-dead direct session cannot perform this read. Authenticate the
    # caller, but do not spend the shared Pinnacle account budget on a request
    # that is guaranteed to fail before touching Pinnacle.
    if session.login_error:
        token = _extract_api_token(request)
        if _API_KEYS and token not in _API_KEYS:
            raise HTTPException(status_code=401, detail="missing or invalid API token")
        return {
            ...
            "error_code": "PS3838_AUTH_FAILED",
            "error": session.login_error,
            "consumer_id": _consumer_id(request),
            "outcome": req.outcome or req.raw_selection,
            "timestamp": time.time(),
        }
    auth = await _authorize_and_rate_limit(request, "market-margin")
    ...
    if context_resolution:
        ...
        resolved_parent_event_id = context_resolution.get("parent_event_id")
    elif market_scope:
        ...
        if event:
            ...
            try:
                ...
                resolved_event_id = int(req.event_id)
            if resolved_event_id != int(req.event_id):
                resolved_parent_event_id = int(req.event_id)
    else:
        sport_id, sport_label, event = await _compact_event_for_market_margin(int(req.event_id), sport_hint)
        source = "compact"
    # An explicit tennis scope must never fall back to the root board.  A
    # missing or ambiguous Games/Sets child is an identity failure, not an
    # invitation to price the nearest event.
    if not event and not context_resolution and not market_scope:
        more_bet_event = await _more_bet_event_for_market_margin(sport_id, int(req.event_id))
        if more_bet_event:
...
_BIA_LOOKUP_ERROR_CODE_RE = re.compile(r"^[A-Z][A-Z0-9_]{1,63}$")
_BIA_LOOKUP_REFRESH_STATUS_RE = re.compile(r"^[A-Za-z][A-Za-z0-9_-]{0,31}$")


def _sanitize_bia_lookup_failure(data: Any) -> dict[str, Any]:
    """Keep only bounded, non-sensitive diagnostics from a failed lookup-bia call."""
    if not isinstance(data, dict) or data.get("found") is True:
        return {}
    ...
    if isinstance(error_code, str):
        ...
        if _BIA_LOOKUP_ERROR_CODE_RE.fullmatch(error_code):
    ...
    if (
        ...
        and 0 <= candidate_count <= 100_000
    ):
        sanitized["candidate_count"] = candidate_count

    refresh_status = data.get("refresh_status")
    if isinstance(refresh_status, str):
        refresh_status = refresh_status.strip().lower()
        if _BIA_LOOKUP_REFRESH_STATUS_RE.fullmatch(refresh_status):
            sanitized["refresh_status"] = refresh_status

    event_found = data.get("event_found")
    if isinstance(event_found, bool):
        sanitized["event_found"] = event_found
...
async def handle_fallback_verify(req: VerifyRequest) -> dict:
    ...
    if bia_enabled and bia_login and bia_password:
        ...
        try:
            ...
            lookup_period = 0 if map_number > 0 else effective_period
    ...
    for field in ("candidate_count", "refresh_status", "event_found"):
        if field in lookup_failure:
            body[field] = lookup_failure[field]
    return {**body, "results": [dict(body, event_id=int(req.event_id), market=req.market)]}
