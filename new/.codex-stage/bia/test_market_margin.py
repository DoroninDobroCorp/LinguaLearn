...
class MarketMarginTests(unittest.TestCase):
    def test_known_auth_failure_does_not_consume_shared_account_rate_budget(self):
        async def forbidden_rate_charge(*_args, **_kwargs):
            raise AssertionError("known auth failure must not enter the account limiter")

        original_cache = ps_app.cache
        original_auth = ps_app._authorize_and_rate_limit
        original_login_error = ps_app.session.login_error
        ps_app.cache = object()
        ps_app._authorize_and_rate_limit = forbidden_rate_charge
        ps_app.session._login_error = "account locked"
        try:
            response = TestClient(ps_app.app).post(
                "/market-margin",
                json={
                    "event_id": 1631000,
                    "sport": "Tennis",
            ...
            ps_app._authorize_and_rate_limit = original_auth
            ps_app.session._login_error = original_login_error

        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(response.json()["error_code"], "PS3838_AUTH_FAILED")
    ...
    def test_market_margin_scoped_child_returns_parent_event_proof(self):
        root = {
            "raw": [100, "Player A", "Player B", {}],
            "home": "Player A",
            "away": "Player B",
        }
        games = {
        ...
        async def compact_root(_event_id, _sport_hint):
            return 33, "Tennis", root

        async def more_bet_root(_sport_id, _event_id):
            return root

        async def allow_request(_request, _scope):
            return ps_app._AuthContext("test", "test-rate")

        original_cache = ps_app.cache
        original_context = ps_app._resolve_contextual_market_request
        original_compact = ps_app._compact_event_for_market_margin
        original_more_bet = ps_app._more_bet_event_for_market_margin
        original_auth = ps_app._authorize_and_rate_limit
        original_login_error = ps_app.session.login_error
        ps_app.cache = object()
        ps_app._resolve_contextual_market_request = no_context
        ps_app._compact_event_for_market_margin = compact_root
        ps_app._more_bet_event_for_market_margin = more_bet_root
        ps_app._authorize_and_rate_limit = allow_request
        ...
        try:
            ...
            ps_app.cache = original_cache
            ps_app._resolve_contextual_market_request = original_context
            ps_app._compact_event_for_market_margin = original_compact
        ...
        self.assertEqual(payload["parent_event_id"], 100)
        self.assertEqual(payload["market_scope"], "games")

    def test_market_margin_missing_or_ambiguous_scope_fails_closed(self):
        missing = {
            "raw": [100, "Player A", "Player B", {"0": [[], [], [1.91, 1.95, None, 777]]}],
            "home": "Player A",
            "away": "Player B",
        }
        ambiguous = {
            **missing,
            "children": [
                {
                    "raw": [101, "Player A (Games)", "Player B (Games)", {}],
                    "home": "Player A (Games)",
                    "away": "Player B (Games)",
                },
                {
                    "raw": [102, "Player A Games", "Player B Games", {}],
                    "home": "Player A Games",
                    "away": "Player B Games",
                },
            ],
        ...
        for name, root in (("missing", missing), ("ambiguous", ambiguous)):
            with self.subTest(name=name):
                ...
                self.assertEqual(payload["event_id"], 100)
    ...
    def test_line_resolver_rejects_neighbouring_structural_lines(self):
        spread_event = _event({
            "0": [
                [[1.49, -1.49, "1.49", 1.91, 1.95, None, None, 5679, 1]],
                [],
                [],
            ]
        })
        total_event = _event({
            "0": [
                [],
                [["2.49", 2.49, 1.91, 1.95, 6789, 1]],
                [],
            ]
        })
        team_total_event = _event({
            "0": [
                [],
                [],
                [],
                None,
                [["2.49", 2.49, 1.91, 1.95, 7890, 1]],
            ]
        })

        self.assertIsNone(resolve_line_meta(
            spread_event, period=0, bet_type=2, team_select=0, handicap=1.5,
        ))
        self.assertIsNone(resolve_line_meta(
            total_event, period=0, bet_type=3, team_select=3, handicap=2.5,
        ))
        self.assertIsNone(resolve_line_meta(
            team_total_event, period=0, bet_type=4, team_select=5, handicap=2.5,
        ))
