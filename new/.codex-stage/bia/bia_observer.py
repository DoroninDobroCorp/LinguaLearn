...
_WATCH_EVENT_RICH_SPORTS = frozenset({
    "fb", "fb_ht", "fb_htft", "fb_corn", "fb_corn_ht", "tennis", "esports",
})
...
def _registry_entry_fingerprint(entry: dict) -> tuple[str, str, int | None]:
    """Structural identity shared by duplicate BIA catalog aliases."""
    from services.bia_event_matcher import _name_variants

    def participant(value: object) -> str:
        variants = _name_variants(str(value or ""))
        return min(variants, key=lambda item: (len(item), item)) if variants else ""

    return (
        participant(entry.get("home")),
        participant(entry.get("away")),
...
def _dedupe_equivalent_event_refs(
    ...
    for item in refs:
        ...
        registry_entry = stats._event_registry.get((
            str(candidate.get("comp_id") or ""),
            str(candidate.get("sport_code") or ""),
            str(candidate.get("event_key") or ""),
        ))
        fingerprint = (
            _registry_entry_fingerprint(registry_entry)
            if isinstance(registry_entry, dict)
            else ("", "", None)
        )
        # BIA occasionally allocates two entity ids for spelling aliases of
        # the same team (for example ``CCG`` / ``CCG Esports``).  Same sport,
        # competition, participant identities and grounded start make those
        # catalog rows one physical event even when the raw event keys differ.
        # Incomplete fingerprints keep the stricter raw-key identity.
        key = (
            str(candidate.get("sport_code") or "").strip().lower(),
            canonical_comp,
            fingerprint
            if all(value not in (None, "") for value in fingerprint)
            else _physical_bia_event_key(candidate.get("event_key")),
        )
...
def _prefer_exact_participant_refs(game: dict, refs: list[dict]) -> list[dict]:
    ...
    target_home = _name_variants(str(
        game.get("homeName") or game.get("Home") or game.get("home") or ""
    ))
    target_away = _name_variants(str(
        game.get("awayName") or game.get("Away") or game.get("away") or ""
    ))

    def score(item: dict) -> int:
        ref_home = _name_variants(str(item.get("home") or ""))
        ref_away = _name_variants(str(item.get("away") or ""))
        if bool(item.get("swapped")):
            ref_home, ref_away = ref_away, ref_home
        return int(bool(ref_home & target_home)) + int(bool(ref_away & target_away))
...
def _configured_discovery_sports() -> list[str]:
    ...
    for raw_sport in getattr(_cfg, "BIA_SPORTS", ()) or ():
        ...
        if sport and sport not in seen:
            sports.append(sport)
            seen.add(sport)
    # Corner boards are structural child namespaces of configured football,
    # not optional unrelated sports. Their rich first-subscription snapshot is
    # required to prove exact corner totals/handicaps later; cpricefeed does
    # not reliably replay that snapshot on a fresh auxiliary socket.
    if "fb" in seen:
        for child_sport in ("fb_corn", "fb_corn_ht"):
            if child_sport not in seen:
                sports.append(child_sport)
                seen.add(child_sport)
    return sports
...
_BIA_SPORT_TO_PERIOD: dict[str, int] = {
    ...
    "fb_htft": 0,
    # Corners have their own BIA namespaces. They share Pinnacle's match/half
    # period numbers but must be selected only by explicit market context.
    "fb_corn": 0,
    "fb_corn_ht": 1,
...
def _filter_bia_event_refs_for_market_context(
    refs: list[dict],
    *,
    market_context: object,
    period: int,
) -> list[dict]:
    """Keep soccer and corner namespaces structurally disjoint.

    Participant identity and period alone are identical for a match and its
    corners board. The explicit context from Forted/Pinnacle is therefore a
    required coordinate; price is deliberately absent.
    """
    context = str(market_context or "").strip().lower()
    corner_codes = {"fb_corn", "fb_corn_ht"}
    if context == "corners":
        expected = "fb_corn_ht" if int(period or 0) == 1 else "fb_corn"
        return [ref for ref in refs if str(ref.get("sport_code") or "") == expected]
    return [ref for ref in refs if str(ref.get("sport_code") or "") not in corner_codes]
    ...
    if len(stats._watch_event_pending) >= target_pending:
        ...
        while (
            ...
            if sport not in _WATCH_EVENT_RICH_SPORTS:
                ...
                scanned += 1
                continue
            derived_corner = sport in {"fb_corn", "fb_corn_ht"} and "fb" in (allowed or set())
            if allowed and sport not in allowed and not derived_corner:
                idx = (idx + 1) % total
                scanned += 1
                continue
            key = (triple[0], sport, triple[2])
...
def lookup_bia_selection_for_pid(
    ...
    *,
    ...
    event_refs = _filter_bia_event_refs_for_market_context(
        event_refs,
        market_context=(selection or {}).get("market_context"),
        period=target_period,
    )
    if not event_refs:
        return {
    ...
    failures: list[dict] = []
    for event_ref in event_refs:
        if str(proof_selection.get("special_type") or "").strip():
            proof = stats._offer_proofs.try_prove_special(
                event_ref,
                special_type=proof_selection.get("special_type"),
                contestant=proof_selection.get("contestant"),
                period=target_period,
                handicap=proof_selection.get("handicap", 0),
            )
        else:
...
def _exact_refresh_key(event_id: int, period: int, selection: dict) -> tuple:
    ...
    return (
        ...
        str(selection.get("market_context") or "").strip().lower(),
        str(selection.get("period_type") or "").strip().lower(),
        _coordinate("inning_number"),
        _coordinate("half_number"),
        str(selection.get("special_type") or "").strip().lower(),
        str(selection.get("contestant") or "").strip(),
    )
...
def _fresh_exact_refresh_result(
    ...
    if current_revision != request.candidate_revision:
        ...
        current_refs = _filter_bia_event_refs_for_market_context(
            current_refs,
            market_context=request.selection.get("market_context"),
            period=request.period,
        )
        current_by_key = {
            (
...
async def _verified_exact_refresh_ws_url(bia: BiaSession) -> str | None:
    """Return the observer token for one bounded auxiliary WS generation.

    ``BiaSession.is_expired`` is a local re-login age, not evidence that the
    server invalidated the token.  The long-lived observer WebSocket can still
    be authenticated after that age.  Requiring an additional HTTP ``verify``
    here made every exact refresh unavailable as soon as its short timeout was
    crossed, even while the main feed remained healthy.

    The auxiliary WebSocket handshake is itself the authoritative token check
    and is already bounded by ``_EXACT_REFRESH_CONNECT_TIMEOUT_SEC``.  An
    invalid token therefore still fails closed in ``_run_exact_refresh_generation``
    without mutating or re-logging the main observer session.
    """
    if not bia.token:
        return None
    return bia.ws_url()
...
async def _run_exact_refresh_generation(
    ...
    try:
        ...
        log(
            "[BIA-obs] exact refresh generation failed: "
...
async def _drain_exact_refresh_requests(
    ...
    *,
    ...
    for (event_id, period), grouped_requests in grouped.items():
        ...
        all_event_refs = _matching_bia_event_refs_for_pid(
            event_id,
            period=period,
            stats=stats,
        )
        ...
        for request in grouped_requests:
            ...
            if _exact_refresh_generation_task is None and not request.done.is_set():
                if (
