...
def list_system_rejections(
    *,
    ...
    rows, _total = get_system_rejections_page(
        now=now,
        limit=limit,
        category=category,
...
def get_system_rejection_coverage(
    *,
    now: float | None = None,
    since: float | None = None,
    category: str | None = "verification",
    limit: int = 100,
) -> tuple[list[dict[str, Any]], dict[str, int]]:
    """Aggregate retained diagnostics without exposing historical forks to clients."""
    ts = time.time() if now is None else float(now)
    ...
    with _diagnostics_lock:
        ...
        totals_row = conn.execute(
            f"""
            SELECT COUNT(*) AS records, COALESCE(SUM(occurrences), 0) AS occurrences
            FROM system_rejections
            WHERE {where_sql}
            """,
            tuple(params),
        ).fetchone()
        rows = conn.execute(
            f"""
            SELECT
                category, error_code, COALESCE(sport, '') AS sport,
                COALESCE(market, '') AS market, COALESCE(stage, '') AS stage,
                COALESCE(source, '') AS source, COUNT(*) AS records,
                COALESCE(SUM(occurrences), 0) AS occurrences,
                MIN(first_seen) AS first_seen, MAX(last_seen) AS last_seen
            FROM system_rejections
            WHERE {where_sql}
            GROUP BY category, error_code, sport, market, stage, source
    ...
    totals = {
        ...
        "occurrences": int(totals_row["occurrences"] or 0),
    }
