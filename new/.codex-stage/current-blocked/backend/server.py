...
async def list_verification_rejections(
    ...
    try:
        items, total_count = await asyncio.to_thread(
            _storage.get_system_rejections_page,
            now=now,
            limit=max(1, min(int(limit), 1000)),
            category=safe_category or None,
            active_after=now - ROBINARB_SYSTEM_REJECTION_ACTIVE_SEC,
        )
    except Exception as exc:
        log.warning("Could not read system rejection diagnostics: %s", exc)
        return {
            "items": [],
            "count": 0,
            "active_count": 0,
...
async def get_arbs(
    ...
    return {
        ...
        "system_blocked_count": len({item["id"] for item in system_rejection_items}),
