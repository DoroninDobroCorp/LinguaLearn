...
async function request(path, options = {}, config = {}) {
  ...
  const { headers: optionHeaders, signal: _unusedSignal, ...fetchOptions } = options;
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), requestTimeoutMs(path, timeoutMs));
  const headers = {
  ...
  try {
    ...
    const wrapped = new Error(timedOut ? formatRequestError(path) : (error?.message || 'Network request failed'));
    wrapped.status = 0;
    wrapped.code = timedOut ? 'REQUEST_TIMEOUT' : 'NETWORK_ERROR';
    throw wrapped;
  } finally {
...
export const api = {
  ...
  getArbs: ({ sport, market, bookmaker, search, minProfit, live, refresh, robinWork } = {}) => {
  ...
  downloadAdminStatsRecordEventsCsv: (recordId) =>
    download(`/admin/stats/records/${encodeURIComponent(recordId)}/events.csv`, `${recordId}_price_changes.csv`),
  ...
  getVerificationRejections: ({ limit = 200, category = 'all' } = {}) => {
    const params = new URLSearchParams();
  ...
  getBookmaker: () => request('/forted/bookmaker'),
