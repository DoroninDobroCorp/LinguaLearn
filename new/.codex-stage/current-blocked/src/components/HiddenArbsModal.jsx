...
export default function HiddenArbsModal({ open, initialTab = 'user', onClose, onRestored, showToast }) {
  ...
  const loadTab = useCallback(async (tab, requestedCategory = 'all', { background = false } = {}) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      loadTab('system', systemCategory, { background: true });
    }, 15_000);
    return () => window.clearInterval(timer);
  }, [open, activeTab, systemCategory, loadTab]);
