...
DEFAULT_OFFER_PROOF_TTL_SEC = 300.0
...
def _parse_raw_group(raw_group: Any) -> _RawGroupSpec | None:
    ...
    if (
        ...
        if parts[1] == "tmap":
            ...
            if map_number <= 0 or map_number > 5 or str(map_number) != parts[2]:
                return None
        return _RawGroupSpec(f"scoped_ml_{parts[1]}", "", parts[2])
    ...
    if (
        ...
        if map_number <= 0 or map_number > 5 or str(map_number) != parts[2]:
            return None
        return _RawGroupSpec("scoped_team_total_tmap", parts[3], str(map_number))
...
def _positional_outcome_names(spec: _RawGroupSpec) -> tuple[str, str] | None:
    if spec.kind in {
        ...
        return "over", "under"
    if spec.kind in {"simple_ah", "scoped_ah_tmap", "scoped_ah_tp"}:
        return "h", "a"
...
_POSITIONAL_HANDICAP_KINDS = frozenset(
    {"simple_ah", "scoped_ah_tmap", "scoped_ah_tp", "tennis_ah"}
)
...
def _parse_standard_selection(selection: Mapping[str, Any]) -> _StandardSelection:
    ...
    if esports_unit == "kills":
        # No raw cpricefeed sub,kills group grammar has been grounded yet.
        # Never let a kills request reuse a structurally similar rounds group.
        raise BiaOfferProofError("BIA_ESPORTS_KILLS_SCOPE_UNPROVEN")
...
def _validate_selection_scope(sport_code: str, selection: _StandardSelection) -> None:
    ...
    if is_esports:
        ...
        if selection.bet_type in {2, 3, 4, 5}:
            ...
            if selection.map_number and selection.esports_unit != "rounds":
...
def _serialise_candidate(
    ...
    *,
    ...
    if simple:
        ...
        if selection.map_number or spec.scope != "all" or sport_code.startswith("fb"):
            return None
        if (
