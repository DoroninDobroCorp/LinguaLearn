...
class PinnacleArcadiaTests(unittest.TestCase):
    ...
    def test_exact_games_matchup_id_beats_related_root_with_same_handicap_key(self):
        root = matchup(1632949639, home="Alexandra Eala", away="Qinwen Zheng")
        games = matchup(
            1632974942,
            home="Alexandra Eala (Games)",
            away="Qinwen Zheng (Games)",
        )
        root_market = market(1632949639, market_type="spread", period=0, prices=[
            {"designation": "home", "points": 1.5, "price": -187},
            {"designation": "away", "points": -1.5, "price": 161},
        ])
        root_market["key"] = "s;0;s;1.5"
        games_market = market(1632974942, market_type="spread", period=0, prices=[
            {"designation": "home", "points": 1.5, "price": 102},
            {"designation": "away", "points": -1.5, "price": -119},
        ])
        games_market["key"] = "s;0;s;1.5"
        data = self.cache([root, games], [root_market, games_market])

        result = pa.lookup_pinnacle(
            "Tennis", "Alexandra Eala", "Qinwen Zheng", "Handicap", "H2 -1.5",
            raw_selection="Ф2(-1,5)", matchup_id=1632974942, cache=data,
        )

        self.assertIsNotNone(result)
        self.assertEqual(result["matchup_id"], 1632974942)
        self.assertEqual(result["matched_home"], "Alexandra Eala (Games)")
        self.assertEqual(result["market_key"], "s;0;s;1.5")
        self.assertEqual(result["designation"], "away")
        self.assertEqual(result["points"], -1.5)
        self.assertAlmostEqual(result["decimal_odds"], 1.8403, places=4)

    def test_exact_matchup_id_fails_closed_instead_of_guessing_related_line(self):
        root = matchup(123, home="Player One", away="Player Two")
        games = matchup(124, home="Player One (Games)", away="Player Two (Games)")
        games_market = market(124, market_type="spread", period=0, prices=[
            {"designation": "away", "points": -4.5, "price": -120},
        ])
        games_market["key"] = "games-spread-4.5"
        data = self.cache([root, games], [games_market])

        result = pa.lookup_pinnacle(
            "Tennis", "Player One", "Player Two", "Handicap", "H2 -4.5",
            raw_selection="Ф2(-4,5)", matchup_id=123, cache=data,
        )

        self.assertIsNone(result)

    def test_spread_side_and_sign_survive_reversed_participant_order(self):
        event = matchup(123, home="Player Two", away="Player One")
        spread = market(123, market_type="spread", period=0, prices=[
            {"designation": "home", "points": -1.5, "price": -120},
            {"designation": "away", "points": 1.5, "price": 110},
        ])
        spread["key"] = "s;0;s;1.5"
        data = self.cache([event], [spread])

        cases = (
            ("Ф2(-1,5)", "home", -1.5, 1.8333),
            ("Ф1(1,5)", "away", 1.5, 2.1),
        )
        for raw_selection, designation, points, decimal_odds in cases:
            with self.subTest(raw_selection=raw_selection):
                result = pa.lookup_pinnacle(
                    "Tennis", "Player One", "Player Two", "Handicap", "",
                    raw_selection=raw_selection, matchup_id=123, cache=data,
                )
                self.assertIsNotNone(result)
                self.assertTrue(result["reversed"])
                self.assertEqual(result["designation"], designation)
                self.assertEqual(result["points"], points)
                self.assertAlmostEqual(result["decimal_odds"], decimal_odds, places=4)
    ...
    def test_common_forted_selection_taxonomy_parses_to_exact_coordinates(self):
        ...
        for raw in (
            "1X", "X2", "12", "0:0", "К1 пройдёт", "ИТ1(8,5)", "IT2 10.5",
    ...
    def test_market_margin_does_not_pair_neighbouring_total_line(self):
        event = matchup(123)
        data = self.cache([event], [market(123, market_type="total", period=0, prices=[
            {"designation": "over", "points": 2.49, "price": -110},
            {"designation": "under", "points": 2.5, "price": -105},
        ])])

        result = pa.lookup_pinnacle(
            "Soccer", "Home FC", "Away FC", "Totals", "",
            raw_selection="ТМ(2,5)", matchup_id=123, cache=data,
        )

        self.assertIsNotNone(result)
        self.assertIsNone(result["market_margin"])
    ...
    def test_duplicate_related_matchups_for_explicit_scope_fail_closed(self):
        direct_games = matchup(124, home="Player One (Games)", away="Player Two (Games)")
        first_sets = matchup(123, home="Player One", away="Player Two")
        second_sets = matchup(125, home="Player One", away="Player Two")
        first_market = market(123, market_type="spread", period=0, prices=[
            {"designation": "away", "points": 1.5, "price": 103},
        ])
        first_market["key"] = "sets-a"
        second_market = market(125, market_type="spread", period=0, prices=[
            {"designation": "away", "points": 1.5, "price": -133},
        ])
        second_market["key"] = "sets-b"
        data = self.cache([direct_games, first_sets, second_sets], [first_market, second_market])

        result = pa.lookup_pinnacle(
            "Tennis", "Player One", "Player Two", "Handicap", "",
            raw_selection="Ф2(1,5)", matchup_id=124, market_scope="sets", cache=data,
        )

        self.assertIsNone(result)

    def test_exact_id_wins_when_it_already_matches_explicit_scope(self):
        direct_sets = matchup(123, home="Player One", away="Player Two")
        other_sets = matchup(125, home="Player One", away="Player Two")
        direct_market = market(123, market_type="spread", period=0, prices=[
            {"designation": "away", "points": 1.5, "price": -133},
        ])
        direct_market["key"] = "direct-sets"
        other_market = market(125, market_type="spread", period=0, prices=[
            {"designation": "away", "points": 1.5, "price": 103},
        ])
        other_market["key"] = "other-sets"
        data = self.cache([direct_sets, other_sets], [direct_market, other_market])
        ...
        self.assertIsNotNone(result)
        self.assertEqual(result["matchup_id"], 123)
        self.assertEqual(result["market_key"], "direct-sets")

    def test_nearby_numeric_line_is_not_treated_as_exact(self):
        event = matchup(123)
        nearby = market(123, market_type="spread", period=0, prices=[
            {"designation": "away", "points": -1.49, "price": -119},
        ])
        nearby["key"] = "nearby-line"
        data = self.cache([event], [nearby])
