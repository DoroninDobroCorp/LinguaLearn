...
def _selection_market_spec(selection: Dict[str, Any]) -> Tuple[Optional[str], Optional[str]]:
    ...
    if bet_type == 4:
        return "FirstTeamTotals", {3: "WinMore", 4: "WinLess"}.get(team_select)
    if bet_type == 5:
        return "SecondTeamTotals", {3: "WinMore", 4: "WinLess"}.get(team_select)
    return None, None
...
# The active parser refreshes this shared snapshot.  If it goes stale the
# account is resting or unauthenticated, so real placement must fail closed.
PARSER_SESSION_MAX_AGE_SEC = float(
    os.environ.get("PS3838_PARSER_SESSION_MAX_AGE_SEC", "180")
)
...
BET_SERVICE_HOSTS = [
    host.strip()
    for host in os.environ.get("BET_SERVICE_HOSTS", "127.0.0.1").split(",")
    if host.strip()
]
...
class PS3838BetClient:
    ...
    def __init__(self, session_file: str, *, own_session: bool = False):
        ...
        self._parser_bia_cache: Dict[Tuple[int, int], Tuple[float, Dict[str, Any]]] = {}
    ...
    def session_owner_state(self) -> Dict[str, Any]:
        age_sec = max(0.0, time.time() - self._session_mtime) if self._session_mtime else None
        limit = self._owned_session_max_age_sec if self._own_session else PARSER_SESSION_MAX_AGE_SEC
        return {
            ...
            "parser_max_age_sec": None if self._own_session else PARSER_SESSION_MAX_AGE_SEC,
            "age_sec": round(age_sec, 3) if age_sec is not None else None,
            "fresh": bool(age_sec is not None and age_sec <= limit),
        }
    ...
    async def _fetch_bia_event_ref(
        ...
        cache_key = (int(event_id), int(period))
        now = time.time()
        ...
        qs = urllib.parse.urlencode({"event_id": event_id, "period": period})
        ...
        try:
            ...
            log.debug("Failed to fetch BIA event ref for %s period=%s: %s", event_id, period, e)
            return None
        ...
        *,
        ...
        for idx, result in enumerate(enriched):
            ...
            try:
                ...
                continue
            period = int(selection.get("period", result.get("period_num", 0)) or 0)
            event_ref = await self._fetch_bia_event_ref(event_id, period)
            if not event_ref:
                result["exact_price"] = {
                    "status": "UNAVAILABLE",
    ...
    async def place_bet(
        ...
        if not self._own_session:
            _data, session_mtime = self._read_session_file()
            session_age = time.time() - session_mtime if session_mtime > 0 else float("inf")
            if session_age > PARSER_SESSION_MAX_AGE_SEC:
                return {
                    "status": "BLOCKED",
                    "error": "PARSER_SESSION_STALE",
                    "session_age_sec": round(session_age, 3) if session_age != float("inf") else None,
                    "max_age_sec": PARSER_SESSION_MAX_AGE_SEC,
                }
            await self._reload_session_from_parser_file(allow_unchanged=True)
        if self._session is None:
            return {"status": "BLOCKED", "error": "PARSER_SESSION_UNAVAILABLE"}
...
async def health_handler(request: web.Request) -> web.Response:
    ...
    # Parser-owned snapshots are atomically replaced by the active fleet worker.
    # Refresh only from that local file so health reflects the session that /place
    # will actually use, without making any bookmaker request.
    if not client._own_session:
        await client._maybe_reload_session()
...
if __name__ == "__main__":
    web.run_app(create_app(), host=BET_SERVICE_HOSTS, port=BET_SERVICE_PORT)
