...
class RobinArbApiTests(unittest.TestCase):
    ...
    def test_verify_uses_price_confirmed_mirror_line_with_cart_handicap(self):
        headers = self.login("owner", "owner123")
        self.stop_background_relay()
        self.assume_forted_stream_alive()
        now = time.time()
        ...
        async def fake_lookup_more_bet_price(**kwargs):
            self.assertEqual(kwargs["expected_decimal_odds"], 1.408)
            return {
                "source": "pinnacle-more-bet",
                "event_id": "1631779",
                "line_id": "56729624961",
                ...
                "team_select": 0,
                "handicap": 2.5,
                "matched_by": "price_confirmed_mirror_handicap",
                "home": "Home FC",
                "away": "Away FC",
        ...
        self.assertEqual(metadata["pinnacle_lookup_matched_by"], "price_confirmed_mirror_handicap")
    ...
    def test_robin_work_selects_top_five_by_recalculated_robin_profit(self):
        ...
        original_ensure_board = server.robin_margin.ensure_board
        original_robin_odds_for = server.robin_margin.robin_odds_for
        original_cached_price_entry = server.robin_margin.cached_price_entry
        original_lookup_stream_price = server.pinnacle_hub.lookup_stream_price
        ensure_calls = []
        odds_calls = []
        ...
        async def fake_ensure_board(event_id, force=False):
            ensure_calls.append(str(event_id))
            return True

        def fake_robin_odds_for(pin_odds, event_id, sport, raw_selection, cache_key=None, price_signature=None):
            odds_calls.append(str(event_id))
            return 1.5 if str(event_id) == "1631000" else 2.3

        def fake_cached_price_entry(cache_key):
            return {"source": "hub-board"}

        server.ROBINARB_ROBIN_WORK_TOP_N = 5
        server.ROBINARB_ROBIN_WORK_CANDIDATE_N = 6
        server.pinnacle_hub.lookup_stream_price = fake_lookup_stream_price
        server.robin_margin.ensure_board = fake_ensure_board
        server.robin_margin.robin_odds_for = fake_robin_odds_for
        server.robin_margin.cached_price_entry = fake_cached_price_entry
        try:
            ...
            server.robin_margin.ensure_board = original_ensure_board
            server.robin_margin.robin_odds_for = original_robin_odds_for
            server.robin_margin.cached_price_entry = original_cached_price_entry
        ...
        self.assertEqual(ensure_calls, [f"163100{idx}" for idx in range(6)])
        ...
        self.assertEqual(by_id["rw-top-5"]["robin_price_source"], "hub-board")
    ...
    def test_robin_work_calculates_top_per_counter_bookmaker_group(self):
        ...
        def fake_robin_odds_for(pin_odds, event_id, sport, raw_selection, cache_key=None, price_signature=None):
            odds_calls.append(str(event_id))
            return 1.5 if str(event_id).endswith("-0") else 2.3
    ...
    def test_robin_work_reuses_cached_margin_when_stream_price_is_unchanged(self):
        headers = self.login("owner", "owner123")
        self.stop_background_relay()
        self.assume_forted_stream_alive()
        now = time.time()
        server._arbs_source = "listener"
        server._arbs_updated_at = now
        cache_key = server.robin_margin.stream_cache_key("1631999", "Tennis", "Home", "Moneyline")
        server.robin_margin._fork_price_cache[cache_key] = {
            "ts": now - server.robin_margin.STREAM_CACHE_TTL - 5,
            "pin_odds": 2.0,
            "robin_odds": 2.18,
            "source": "hub-board",
            "price_signature": "same-pin-market",
        }
        server._arbs_cache = [{
            "id": "rw-cache-hit",
            "sport": "Tennis",
        ...
        async def raising_ensure_board(*_args, **_kwargs):
            raise AssertionError("unchanged stream price should use cached Robin margin")

        def raising_robin_odds_for(*_args, **_kwargs):
            raise AssertionError("unchanged stream price should not recalculate Robin margin")
        ...
        self.assertEqual(arb["robin_price_source"], "hub-board")
    ...
    def test_robin_work_does_not_select_fallback_margin_rows(self):
        ...
        self.assertEqual(response.status_code, 200, response.text)
        payload = response.json()
        self.assertEqual(payload["robin_work"], {"enabled": True, "top_n": 5, "selected": ["rw-fallback-only"]})
        arb = payload["arbs"][0]
        self.assertTrue(arb["robin_work_selected"])
        self.assertEqual(arb["robin_work_rank"], 1)
        self.assertEqual(arb["robin_price_source"], "stream-fallback")

    def test_robin_work_uses_stream_margin_without_overwriting_forted_price(self):
        headers = self.login("owner", "owner123")
        self.stop_background_relay()
        self.assume_forted_stream_alive()
        now = time.time()
        ...
        async def raising_compact_margin(*_args, **_kwargs):
            raise AssertionError("stream margin should avoid PS3838 compact fallback")

        server.pinnacle_hub.lookup_stream_price = fake_lookup_stream_price
        server.robin_margin.ensure_board = raising_ensure_board
        server._pinnacle_compact_margin_price_for_robin_work = raising_compact_margin
        try:
            response = self.client.get("/api/arbs?robin_work=1", headers=headers)
        finally:
            server.pinnacle_hub.lookup_stream_price = original_lookup_stream_price
        ...
        arb = response.json()["arbs"][0]
        self.assertTrue(arb["robin_work_selected"])
        self.assertEqual(arb["robin_price_source"], "pinnacle-stream")
        self.assertEqual(arb["robin_odds"], round(server.robin_margin.compute_robin_odds(2.0, 0.05), 3))
