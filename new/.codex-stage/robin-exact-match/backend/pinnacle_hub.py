...
def _line_matches(row_line: Any, want_line: Any) -> bool:
    ...
    if row_value is None or want_value is None:
        return False
    return abs(abs(row_value) - abs(want_value)) <= 0.05
...
def _signed_line_matches(row_line: Any, want_line: Any) -> bool:
    ...
    if row_value is None or want_value is None:
        return False
    return abs(row_value - want_value) <= 0.05
...
def _same_stream_market_group(row: list[Any], candidate: list[Any]) -> bool:
    ...
    if market_code == 2:
        ...
        return _signed_line_matches(candidate_line, row_line) if same_runner else abs(candidate_line + row_line) <= 0.05
...
def _approx_eq(a: Any, b: Any, tol: float = 0.01) -> bool:
    fa = _to_float(a)
    fb = _to_float(b)
    ...
    *,
    ...
    if bt == 2:
        ...
        for line in hcp_block:
            ...
            h1 = _to_float(line[0])
            h2 = _to_float(line[1])
            actual = h1 if ts == 0 else h2 if ts == 1 else None
            if actual is None:
                continue
...
async def lookup_stream_price(
    *,
    ...
    # Forted odds are deliberately absent from this resolver.  A selection-only
    # result is diagnostic; authoritative callers must require a Pinnacle id.
