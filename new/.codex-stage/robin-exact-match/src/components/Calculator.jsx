...
export default function Calculator({ arb, balance, onClose, onBetPlaced, showToast, variant, verifyMode = 'betslip', autoAccept = loadAutoAccept() }) {
  ...
  const isRobinCalculating = !arb.robin_work_selected && !verified;
  ...
  }, [verified, verifiedAt, pendingAccept, placing, confirmChange, activeCalc, robinDisplay]);
  ...
  const instantAccept = verifyMode !== 'betslip';
  let statusText;
  let statusColor;
  if (instantAccept) {
    ...
    <div className={wrapClass} onClick={handleWrapClick}>
      <div className={innerClass} onClick={(event) => event.stopPropagation()}>
        ...
        <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: 4 }}>
          <div>{arb.sport} · {arb.market} · Robin {robinDisplay.toFixed(3)}</div>
        ...
        {(currentEdge.pin || currentEdge.robin) && (
          <div className="calc-edge-strip" title="Current arb size at the latest shown prices">
            ...
            <div className={`calc-edge-card robin ${edgeTone(currentEdge.robin?.roiPct)} ${edgePulse.robin ? `pulse-${edgePulse.robin}` : ''}`}>
              <div className="calc-edge-label">
                <span>Robin {isRobinCalculating ? <span className="robin-spin-icon" style={{ display: 'inline-flex', animation: 'spin 1s linear infinite', marginLeft: 4 }}>🔄</span> : <span style={{ color: 'var(--positive)', marginLeft: 4 }}>✅</span>}</span>
                <span>@{isRobinCalculating ? '...' : robinDisplay.toFixed(3)}</span>
              </div>
              <div className="calc-edge-main">{isRobinCalculating ? '⏳...' : signedPct(currentEdge.robin?.roiPct)}</div>
              <div className="calc-edge-sub">
                {isRobinCalculating ? 'расчет...' : signedMoney(currentEdge.robin?.net)}
                <span>Robin {isRobinCalculating ? '$...' : `$${formatStake(currentEdge.robin?.primaryStake, 2)}`}</span>
              </div>
              <div className="calc-edge-meta">
                <span>{edgeAgeText}</span>
              </div>
          ...
          <div className="calc-grid-compact">
            ...
            <div className="calc-section robin">
              <div className="calc-section-head" style={{ color: 'var(--positive)' }}>
                ...
                >
                  {isRobinCalculating ? '⏳...' : `${activeCalc.robinbet.profit >= 0 ? '+' : ''}$${Number(activeCalc.robinbet.profit).toFixed(2)}`}
                </span>
              </div>
              <OutcomePill pick={pinOutcomeText} accent="var(--positive)" />
              <div className="calc-inline" style={{ opacity: isRobinCalculating ? 0.4 : 1, transition: 'opacity 0.2s' }}>
                <span>{isRobinCalculating ? '$...' : `$${activeCalc.robinbet.stake}`}</span>
                <span className="dim">@</span>
                <span>{isRobinCalculating ? '...' : Number(activeCalc.robinbet.odds).toFixed(3)}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="action-buttons action-buttons-large">
          ...
          <button
            ...
            disabled={!activeCalc || waiting || isRobinCalculating}
          >
            {isRobinCalculating ? (
