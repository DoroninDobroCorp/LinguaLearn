...
def _arcadia_structural_raw_selection(raw_selection: str, metadata: dict[str, Any]) -> str:
    ...
    team_total_family = _normalize_market_name(raw_family) in {"teamtotal", "teamtotals", "individualtotal"}
    line = _to_float_or_none(metadata.get("line"))
    team = str(metadata.get("team") or "").strip()
    direction = str(metadata.get("direction") or "").strip().lower()
...
async def _pinnacle_compact_margin_price_for_robin_work(
    ...
    *,
    ...
    arb["robin_work_verified_market_key"] = signature
...
async def _pinnacle_exact_verify_price_for_robin_work(
    ...
    *,
    ...
    for candidate in results:
        ...
        arb["robin_work_verified_market_key"] = market_key
...
async def _robin_work_price_for_arb(arb: dict[str, Any]) -> tuple[float, str]:
    ...
    if arcadia_odds is not None and arcadia_odds > 1 and arcadia_margin is not None and arcadia_margin >= 0 and arcadia_key:
        ...
        arb["robin_work_verified_market_key"] = arcadia_key
    ...
    if stream_odds is not None and stream_odds > 1 and stream_margin is not None and stream_margin >= 0 and stream_signature:
        ...
        arb["robin_work_verified_market_key"] = stream_signature
        arb["robin_work_verification_blocked"] = False
        arb["robin_work_verification_block_reason"] = None
