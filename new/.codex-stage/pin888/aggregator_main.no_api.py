...
def _enqueue_remote_event(item: dict[str, Any]) -> dict[str, Any]:
    ...
    is_live_dom = (
        str(item.get("transport") or "") == "authenticated_dom"
        and isinstance(payload, dict)
        and bool(payload.get("isLive"))
    )
    if is_live_dom:
    ...
    if event_id and source_id:
        target_latest = _remote_live_event_latest if is_live_dom else _remote_event_latest
        other_latest = _remote_event_latest if is_live_dom else _remote_live_event_latest
        target_order = (
            _remote_live_event_source_order if is_live_dom else _remote_event_source_order
        )
        with _remote_event_latest_lock:
