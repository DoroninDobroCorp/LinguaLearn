...
func canonicalizePinnacleMarkets(period entity.PeriodData, periodIdx int, homeScore, awayScore int, sportName string) map[string]PinnacleCanonical {
	...
	for line, hcp := range period.Handicap {
		...
		// Find inverted line entry (may have different key format in period.Handicap)
		invertedHcp, hasInverted := period.Handicap[invertedLine]
		if !hasInverted {
			for altKey, altHcp := range period.Handicap {
				if normalizeTotal(altKey) == invertedLine {
					invertedHcp = altHcp
					hasInverted = true
					break
				}
	...
	// Draw No Bet → canonical H 0 at ANY score.
	// DNB Home ≡ H1 0 (home_total > away_total → win, equal → push) — mathematical
	// identity in the absolute/full-match convention used by all parsers.
	// PS3838 converts remaining→absolute in parse_spreads_into (use_score_adjustment=True).
	// updateIfBetter picks the highest reference price from Handicap["0"] vs DNB.
	if period.DrawNoBet != nil {
		if period.DrawNoBet.Home.Value > 0 {
			updateIfBetter(result, prefix+"H1 0", period.DrawNoBet.Home, prefix+"DNB 1")
		}
		if period.DrawNoBet.Away.Value > 0 {
			updateIfBetter(result, prefix+"H2 0", period.DrawNoBet.Away, prefix+"DNB 2")
		}
	}
...
func validateHandicapPairs(
	...
	for line, hcp := range handicapMap {
		...
		invertedHcp, hasInverted := handicapMap[invertedLine]
		if !hasInverted {
			for k, v := range handicapMap {
				if normalizeTotal(k) == invertedLine {
					invertedHcp = v
					hasInverted = true
					break
				}
...
func extractAllDonorMarkets(period entity.PeriodData, periodIdx int, homeScore, awayScore int, sportName string) []DonorMarketWithCanonical {
	...
	// Draw No Bet → canonical H 0 at ANY score (absolute convention, see canonicalizePinnacleMarkets).
	if period.DrawNoBet != nil {
		if period.DrawNoBet.Home.Value > 0 {
			markets = append(markets, DonorMarketWithCanonical{
