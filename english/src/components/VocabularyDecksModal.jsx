...
export default function VocabularyDecksModal({ isOpen, onClose, onDecksCreated }) {
  ...
  const fetchCatalogs = async () => {
    ...
    try {
      const res = await fetch('/english/api/vocabulary/frequency-catalogs', { credentials: 'include' });
      if (res.ok) {
  ...
  const handleGenerateDecks = async () => {
    setGenerating(true);
    try {
      const res = await fetch('/english/api/vocabulary/generate-decks', { credentials: 'include' }, {
        method: 'POST',
