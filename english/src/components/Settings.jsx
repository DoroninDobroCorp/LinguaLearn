...
function Settings() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('/english/api/settings');
  ...
  const saveSettings = async () => {
    try {
      await fetch('/english/api/settings', {
        method: 'POST',
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-4 p-4 bg-gradient-to-r from-yellow-50 to-lime-50 rounded-xl border-2 border-yellow-200">
              <div className="flex items-start space-x-3">
                <Info className="h-5 w-5 text-yellow-700 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="font-semibold text-yellow-900 mb-1">{maxLevel}</p>
                  <p className="text-sm text-yellow-800">{LEVEL_DESCRIPTIONS[maxLevel]}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-yellow-400 to-lime-400 text-yellow-900 rounded-xl hover:from-yellow-500 hover:to-lime-500 transition-all shadow-md hover:shadow-lg font-semibold"
            >
