...
function Chat() {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      try {
        const response = await fetch('/english/api/chat/history');
        ...
        if (data.history.length > 0) {
          ...
          setMessages([{
            role: 'assistant',
            content: 'Hey there! I\'m your English learning assistant. Ready to help you practice, give you exercises, or just chat in English. What would you like to work on today?'
          }]);
        }
      } catch (error) {
  ...
  const clearChat = async () => {
    ...
    try {
      await fetch('/english/api/chat/clear', { method: 'DELETE' });
      setMessages([
        {
          role: 'assistant',
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] bg-white rounded-2xl shadow-2xl overflow-hidden relative">
      ...
      <div className="bg-gradient-to-r from-yellow-300 to-lime-300 px-6 py-4 flex justify-between items-center">
        ...
        <button
          onClick={clearChat}
          className="flex items-center space-x-2 px-3 py-2 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 rounded-lg transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div
                className={`max-w-[70%] rounded-2xl px-5 py-3 shadow-md ${
                  msg.role === 'user'
                    ...
                    : 'bg-gradient-to-r from-lime-300 to-lime-200 text-lime-900'
                }`}
              >
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-gradient-to-r from-lime-300 to-lime-200 text-lime-900 rounded-2xl px-5 py-3 shadow-md">
      ...
      {/* Input */}
      <div className="bg-gradient-to-r from-yellow-200 to-lime-200 px-6 py-4">
        <div className="flex space-x-3">
          <textarea
            ...
            className="flex-1 resize-none rounded-xl px-4 py-3 bg-white border-2 border-yellow-300 focus:border-yellow-400 focus:outline-none shadow-sm"
            rows="1"
            disabled={loading}
          />
          <button
            ...
            className="px-6 py-3 bg-gradient-to-r from-yellow-400 to-lime-400 text-yellow-900 rounded-xl hover:from-yellow-500 hover:to-lime-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"
          >
