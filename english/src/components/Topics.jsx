...
function Topics() {
  ...
  const fetchTopics = async () => {
    try {
      const response = await fetch('/english/api/topics');
  ...
  const deleteTopic = async (id) => {
    ...
    try {
      await fetch(`/english/api/topics/${id}`, { method: 'DELETE' });
      fetchTopics();
    } catch (error) {
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Max level: <span className="font-semibold text-yellow-700">{maxLevel}</span>
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <select
              ...
              className="px-4 py-2 bg-yellow-100 border-2 border-yellow-300 rounded-lg focus:outline-none focus:border-yellow-400"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-r from-yellow-100 to-yellow-200 rounded-xl p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm text-yellow-700">Total topics</p>
                <p className="text-2xl font-bold text-yellow-900">{topics.length}</p>
              </div>
            </div>
          </div>
        ...
        <div key={category} className="bg-white rounded-2xl shadow-2xl p-6">
          ...
          <div className="space-y-3">
            {categoryTopics.map((topic) => (
              <div
                key={topic.id}
                className="bg-gradient-to-r from-yellow-50 to-lime-50 rounded-xl p-5 hover:shadow-md transition-shadow border-2 border-yellow-200"
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-yellow-400 to-lime-400 transition-all duration-500 rounded-full"
