...
public enum AssessmentTier: String, Codable, Equatable, Sendable {
...
public struct PopupPresentationPolicy: Sendable {
    public static func displayMode(for response: WritingAnalyzeResponse, isPreview: Bool) -> PopupDisplayMode {
        if isPreview {
            return .largeCard
        }
...
public struct WritingAnalyzeResponse: Codable, Equatable, Sendable {
    ...
    public var effectiveAssessmentTier: AssessmentTier {
        if let assessment, let tier = AssessmentTier(rawValue: assessment) {
            return tier
        }
        if !errors.isEmpty || topicEvidence.contains(where: { $0.outcome == "error" }) {
            return .clearError
        }
        return .correct
    }
    ...
    public var isClearError: Bool {
        effectiveAssessmentTier == .clearError
    }
    ...
    private enum CodingKeys: String, CodingKey {
        case schemaVersion, eventId, sampleId, previewOnly, accepted, rejectionReason, sourceApp, originalText, correctedText, changed, summaryRu, errors, topicEvidence, topicChanges, assessment
    }
